...
function movieTrailer(movieId) {
  return axios
    ...
    .then(response => {
      ...
      refs.player.src = `http://www.youtube.com/embed/${response.data.results[0].key}?enablejsapi=1&origin=http://example.com`;
      // refs.movieTrailer.href = `https://www.youtube.com/watch?v=${response.data.results[0].key}`;
    });
}
